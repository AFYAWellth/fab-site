...
- ssh into the server
    - For Mac and Linux: `ssh root@flowergirlandbirdie.com`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo sperea10`
    - replace sperea10 with your own
- create password
    - `passwd sperea10`
- Move .ssh key file
    - `cp -r .ssh /home/sperea10`
- change ownership of the .ssh directory
    - `cd /home/sperea10`
    - `chown -R sperea10:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh sperea10@flowergirlandbirdie.com`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- reboot
    - `sudo reboot`
- ssh back in
- `ssh sperea10@flowergirlandbirdie.com`
...
- create a directory for your deployment
    - `sudo mkdir /var/www/flowergirlandbirdie.com/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/flowergirlandbirdie.com/html`
...
- `sudo vim /etc/nginx/sites-available/flowergirlandbirdie.com`
...
server {
    ...
    root /var/www/flowergirlandbirdie.com/html;
    index index.html index.htm index.nginx-debian.html;

    server_name flowergirlandbirdie.com www.flowergirlandbirdie.com;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/flowergirlandbirdie.com /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/sperea10/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/sperea10/.ssh/github_deploy_key.pub >> /home/sperea10/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/sperea10/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H flowergirlandbirdie.com >> ~/.ssh/known_hosts 
         scp -r ./dist/* sperea10@flowergirlandbirdie.com:/var/www/flowergirlandbirdie.com/html
```
